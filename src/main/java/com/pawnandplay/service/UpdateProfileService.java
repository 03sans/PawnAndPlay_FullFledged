...
public class UpdateProfileService {
	...
	public Boolean updateUser(UserModel userModel, String oldUsername) {
		String updateQuery = "UPDATE User SET Firstname = ?, Lastname = ?, Username = ?, Email = ?, Phone = ?, Password = ?, Image = ?"
	    ...
	    try (Connection dbConn = DbConfig.getDbConnection();
	    	...
	    	updateStmt.setString(7, userModel.getImage());  

	    	if (userModel.getDob() != null) {
	    	    updateStmt.setDate(8, Date.valueOf(userModel.getDob()));
	    	    updateStmt.setString(9, oldUsername);
	    	} else {
